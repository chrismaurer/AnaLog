...
def logExceptions(gwFlavour):
    logExceptions = [
        ...
        # Ignore "system error occured on 'connect'" messages.
        ...
        re.compile( r'.*priceproxy.exe/PROD | .... | WARNING | 10004009 | DHEncryptParam: No encryption performed.*', re.I ),\
    ...
    elif 'BTEC' in gwFlavour: logExceptions.extend(BTECExceptions())
    elif 'ICE' in gwFlavour: logExceptions.extend(ICEExceptions())
    elif 'CME' in gwFlavour or 'CBOT' in gwFlavour: logExceptions.extend(FIXExceptions())
    elif 'LME' in gwFlavour: logExceptions.extend(LMEExceptions())
    elif 'MEFF' in gwFlavour: logExceptions.extend(MEFFExceptions())
    elif 'NYSE_Liffe' in gwFlavour: logExceptions.extend(NYSE_LiffeExceptions())
    elif any(gwFlavourName in gwFlavour for gwFlavourName in ['NYSE_Liffe_US', 'TFX', 'TSE']):
        logExceptions.extend(LIFFEConnectExceptions())
    logExceptions.extend(FillServerExceptions())
    logExceptions.extend(TTMExceptions())
    return logExceptions

def FillServerExceptions():
    FillServerExceptions = [
...
def MEFFExceptions():
    ...
    return MEFFExceptions

def NYSE_LiffeExceptions():
    NYSE_LiffeExceptions = [
    ...
    return NYSE_LiffeExceptions

def LIFFEConnectExceptions():
    LIFFEConnectExceptions = [
        # Ignore "Missing backup IP or Port" messages
        re.compile( r'.*Missing backup IP or Port.*', re.I )
        ]
    return LIFFEConnectExceptions